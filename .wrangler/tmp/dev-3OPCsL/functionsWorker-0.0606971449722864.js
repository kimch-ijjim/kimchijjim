...
// .wrangler/tmp/pages-fumLMQ/functionsWorker-0.0606971449722864.mjs
...
async function fetchCsv(sym) {
  ...
  if (!/^Date,Open,High,Low,Close,Volume/i.test(text.trim())) return null;
  return text;
}
__name(fetchCsv, "fetchCsv");
__name2(fetchCsv, "fetchCsv");
function normalizeSymbol(input) {
  let s = String(input || "").trim().toLowerCase();
  if (!s.includes(".") && /^[a-z]+$/.test(s)) s = `${s}.us`;
  return s;
}
__name(normalizeSymbol, "normalizeSymbol");
__name2(normalizeSymbol, "normalizeSymbol");
async function onRequestGet({ request }) {
  try {
    const url = new URL(request.url);
    const raw = url.searchParams.get("symbol") || "AAPL";
    const primary = normalizeSymbol(raw);
    const fallback = primary.replace(/\.us$/, "");
    let csv = await fetchCsv(primary);
    if (!csv) csv = await fetchCsv(fallback);
    if (!csv) {
    ...
    const candles = lines.map((line) => line.split(",")).filter((row) => row.length >= 6 && row[idx.close]).map((row) => ({
      t: (/* @__PURE__ */ new Date(row[idx.date] + "T00:00:00Z")).getTime(),
      o: Number(row[idx.open]),
      h: Number(row[idx.high]),
      l: Number(row[idx.low]),
      c: Number(row[idx.close]),
      v: Number(row[idx.volume])
...
var SRC = "https://old.nasdaqtrader.com/dynamic/SymDir/nasdaqlisted.txt";
...
function calcRSI(closes, period = 14) {
  ...
  const rsi = new Array(closes.length).fill(void 0);
...
__name(calcRSI, "calcRSI");
__name2(calcRSI, "calcRSI");
...
// .wrangler/tmp/bundle-TzS2Ge/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TzS2Ge/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.0606971449722864.js.map
